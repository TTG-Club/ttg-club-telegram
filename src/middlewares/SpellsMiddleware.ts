...
//   getSpellMessage = (
//     spell: NSpell.ISpell,
//     removeDescriptionHTML = false
//   ): {
//     messages: string[];
//     url: string;
//   } => {
//     const messages: string[] = [
//       `<b>${spell.name}</b> [<i>${spell.englishName}</i>]`
//     ];
//
...
//
//     updateMsg(
//       `\n<i>${this.getLevel(spell.level)}, ${this.getSchool(spell.school)}${
//         spell?.meta?.ritual ? ' (ритуал)' : ''
//       }</i>\n`
//     );
//
//     updateMsg(
...
//       this.getEntries(spell.entriesHigherLevel.entries).forEach(
//         (str, index) => {
...
//   };
//
//   getDamageInflicts = (list: string[]): string =>
//     list
//       .map((item: string) =>
...
//       .join('; ');
//
//   getSchool = (name: string): string =>
//     name in SCHOOLS ? SCHOOLS[name as keyof typeof SCHOOLS] : name;
//
//   getLevel = (level: number): string =>
//     level ? `${level} уровень` : 'Заговор';
//
//   getClass = (name: string): string =>
//     name in CLASSES ? CLASSES[name as keyof typeof CLASSES] : name;
//
//   getSource = (source: string): string =>
//     source in SOURCES ? SOURCES[source as keyof typeof SOURCES] : source;
//
...
//
//       const fromStr =
//         from in CLASSES_FROM
//           ? CLASSES_FROM[from as keyof typeof CLASSES_FROM]
//           : from;
//
//       const str = list
//         .map((classObj: NSpell.IClassItem) => {
...
//   };
//
//   getTimes = (times: NSpell.ITime[]): string =>
//     times.map(time => this.getTime(time)).join('; ');
//
...
//
//   getDuration = (durationList: NSpell.IDuration[]): string =>
//     durationList.map(duration => duration.raw.toLowerCase()).join('; ');
//
...
//     ];
//
//     return entries.map((str: string) =>
//       stripHtml(str, { ignoreTags: tags }).result.replace(
//         /href="\//g,
...
//   };
//
//   getOriginal = (engName: string) =>
//     `${process.env.BASE_URL}/spells/${engName.replace(/\s/g, '_')}`;
// }
