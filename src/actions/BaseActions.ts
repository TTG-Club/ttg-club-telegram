...
bot.start(async ctx => {
  try {
    await ctx.reply('Приветствую, искатель приключений! 👋🏻', {
      reply_to_message_id: ctx.message?.message_id,
      reply_markup: Markup.inlineKeyboard([[Markup.callbackButton('Список команд', COMMAND_NAME.HELP)]])
    });
  } catch (err) {
    console.error(err);
  }
...
bot.command(COMMAND_NAME.ABOUT, async ctx => {
  const getLinksKeyboard = () => Markup
    .inlineKeyboard(Object
      .values(SOCIAL_LINKS)
      .map(link => ([Markup.urlButton(link.label, link.url)])));
