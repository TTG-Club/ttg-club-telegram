...
import type { CallbackButton } from "telegraf/typings/markup.js";
...
const EXIT_BUTTON: CallbackButton[] = [
  Markup
    .callbackButton('Закончить поиск заклинания', CALLBACK_ACTIONS.ExitFromSearch)
];
  ...
  try {
    for (let i = 0; i < messages.length; i++) {
      ...
      if (i === messages.length - 1) {
        extra = {
          ...extra,
          reply_markup: Markup.inlineKeyboard([[Markup.urlButton('Оригинал на TTG Club', url)], EXIT_BUTTON])
        };
      }
    ...
    await ctx.reply('Произошла ошибка, поэтому я выслал тебе сырую версию сообщения заклинания... '
      ...
      reply_markup: Markup.inlineKeyboard([
        [
          Markup
            .urlButton(SOCIAL_LINKS.discord.label, SOCIAL_LINKS.discord.url)
        ]
      ]),
      disable_notification: true
    });
